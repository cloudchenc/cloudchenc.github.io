...
class Solution {
    public boolean validateStackSequences(int[] pushed, int[] popped) {
        ...
        int i = 0;
        for (int num : pushed) {
            stack.push(num); // 遍历pushed，顺序压栈
            // 根据 压入栈的所有数字均不相等，所以当 栈顶元素 是否与 popped 中第 i 个元素相同，则判定出栈
            while (!stack.isEmpty() && stack.peek() == popped[i]) {
                stack.pop();
                i++;
            }
        }
        return stack.isEmpty(); // 最后栈空，则入栈、出栈完成
    }
}
```
    时间复杂度：O(N)
    空间复杂度：O(N)
    执行耗时:3 ms,击败了34.48% 的Java用户
	内存消耗:38.1 MB,击败了63.11% 的Java用户